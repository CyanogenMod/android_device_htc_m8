...
static int tfa9887_startup(struct tfa9887_amp_t *amp)
{
    ...
    error = tfa9887_write_reg(amp, 0x09, 0x0002);
    if (0 == error) {
        error = tfa9887_read_reg(amp, 0x09, &value);
    }
    if (0 == error) {
        /* DSP must be in control of the amplifier to avoid plops */
        value |= TFA9887_SYSCTRL_SEL_ENBL_AMP;
        error = tfa9887_write_reg(amp, 0x09, value);
    }

    /* some other registers must be set for optimal amplifier behaviour */
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x40, 0x5A6B);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x05, 0x13AB);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x06, 0x001F);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, TFA9887_SPKR_CALIBRATION, 0x0C4E);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x09, 0x025D);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x0A, 0x3EC3);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x41, 0x0308);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x48, 0x0180);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x49, 0x0E82);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x52, 0x0000);
    }
    if (0 == error) {
        error = tfa9887_write_reg(amp, 0x40, 0x0000);
    }
